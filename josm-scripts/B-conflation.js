...
for (var i = 0; i < josm.layers.length; i++) {
    ...
    if (
        ...
        (!layerPattern || layer.name.contains(layerPattern)) &&
        layer.name.startsWith("Données OSM pour ")
    ) {
        osmLayer = layer;
    }
}
...
function getConflationDialog() {
    ...
    for (var dialogIdx = 0; dialogIdx < dialogs.size(); dialogIdx++) {
        ...
        if (
            dialog.getClass().toString() ==
            "class org.openstreetmap.josm.plugins.conflation.ConflationToggleDialog"
        ) {
            return dialog;
        }
    }
    ...
    josm.alert(
        "Impossible de trouver l'onglet Conflation. Est-ce que le plugin est installé ?"
    );
    return null;
}

function startConflation(referenceDataSet, subjectDataSet) {
    var conflationClassLoader = org.openstreetmap.josm.plugins.PluginHandler.getPluginClassLoader(
        "conflation"
    );
    ...
    var settingsClass = conflationClassLoader.loadClass(
        "org.openstreetmap.josm.plugins.conflation.SimpleMatchSettings"
    );
    ...
    conflationClassLoader
        .loadClass(
            "org.openstreetmap.josm.plugins.conflation.config.MatchingPanel"
        )
        .getConstructor(
        ...
        .newInstance(null, prefs, null)
        .fillSettings(settings);
    const mergingPanel = conflationClassLoader
        .loadClass(
            "org.openstreetmap.josm.plugins.conflation.config.MergingPanel"
        )
        .getConstructor(
            org.openstreetmap.josm.gui.tagging.ac.AutoCompletionList,
            org.openstreetmap.josm.spi.preferences.IPreferences
        )
        .newInstance(null, prefs);
    ...
    if (dialog != null) {
        ...
        if (!dialog.isVisible()) {
            dialog.unfurlDialog();
            josm.alert(
                "Veuillez maintenant résoudre la conflation, passer enfin à la validation des erreurs potentielles."
            );
        }
    }
}
...
function do_work() {
    ...
    var utils2Plugin = utils2Classloader.loadClass(
        "org.openstreetmap.josm.plugins.utilsplugin2.selection.NodeWayUtils"
    );
    var inside = utils2Plugin
        .getMethod(
            "selectAllInside",
            java.util.Collection,
            org.openstreetmap.josm.data.osm.DataSet
        )
        .invoke(null, ds2.getSelected(), ds2);
    ...
    if (ds2.selectionEmpty()) {
        josm.alert(
            "Il ne semble y avoir aucun bâtiment actuellement dans cette ville. Aucune conflation à réaliser, il suffit d'importer les bâtiments du calque " +
                housesLayer.name
        );
        return;
    }
