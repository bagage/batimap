...
logging.basicConfig(
    ...
    datefmt="%H:%M:%S",
    level=verbosity[
        os.environ.get("INITDB_VERBOSITY")
        or config["DEFAULT"]["VERBOSITY"]
        or "CRITICAL"
    ],
)

BACKEND_DEPARTMENTS_IN_BBOX_URL=config["DEFAULT"]["BACKEND_URL"] + '/departments/in_bbox/{lonNW}/{latNW}/{lonSE}/{latSE}'
...
class Handler(FileSystemEventHandler):
    ...
    def on_any_event(event):
        if event.is_directory:
            ...
            LOG.debug(f"Running initdb on {departments}")
