...
if [ ! -f stats/026-list.txt ]; then
    wget -O stats/026-list.txt http://cadastre.openstreetmap.fr/data/026/026-liste.txt
fi
...
for insee in "$@"; do
    insee="$1"
    ville=${insee/26/CL}-$(grep "${insee/26/CL}" stats/026-list.txt | cut -d '"' -f2)
    ...
    curl 'http://cadastre.openstreetmap.fr/' \
        ...
        --data "dep=026&recherche_dep=drome&ville=$ville&recherche_ville=$insee&bis=true&type=bati&force=false"

    wget "http://cadastre.openstreetmap.fr/data/026/${ville}.tar.bz2"
done